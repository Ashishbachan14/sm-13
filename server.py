            ...
            try:
                ...
                if isinstance(data, dict):
                    if data.get("type") == "collision" and data.get("collision"):
                        collision_count += 1
                        # Optional: Broadcast collision if needed, but not necessary for your setup
                    # NEW: Handle capture request from controller
                    if data.get("type") == "capture_image":
                        broadcast({"command": "capture_image"})
                    # NEW: Forward image responses to all connected clients (e.g., controller)
                    if "image" in data:
                        broadcast(data)
            except Exception:
                pass
...
@app.route('/capture_top_view', methods=['POST'])
def capture_top_view():
    msg = {"command": "capture_top_view"}
    if not broadcast(msg):
        return jsonify({'error': 'No connected simulators.'}), 400
    return jsonify({'status': 'top view capture command sent', 'command': msg})
